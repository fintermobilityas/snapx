...
public static class LogExtensions
{
    ...
    static void GuardAgainstNullLogger(ILog logger)
    {
        if (logger == null) throw new ArgumentNullException("logger");
    }

    static void LogFormat(this ILog logger, LogLevel logLevel, string message, params object[] args)
    {
        logger.Log(logLevel, message.AsFunc(), null, args);
    }

    // Avoid the closure allocation, see https://gist.github.com/AArnott/d285feef75c18f6ecd2b
    static Func<T> AsFunc<T>(this T value) where T : class
    {
        return value.Return;
    }
    ...
    static Func<string> WrapLogInternal(Func<string> messageFunc)
    {
        return messageFunc;
    }
}
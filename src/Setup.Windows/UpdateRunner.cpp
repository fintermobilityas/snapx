...
bool CUpdateRunner::DirectoryIsWritable(wchar_t * szPath)
{
		...
		UINT uRetVal = GetTempFileNameW(szPath, L"Snap", 0, szTempFileName);
		if (uRetVal == 0) {
			return false;
		}
...
int CUpdateRunner::ExtractUpdaterAndRun(wchar_t* lpCommandLine, bool useFallbackDir)
{
	...
	wchar_t* envSnapTemp = _wgetenv(L"SNAP_TEMP");
	if (envSnapTemp &&
		DirectoryExists(envSnapTemp) &&
		DirectoryIsWritable(envSnapTemp) &&
		!PathIsUNCW(envSnapTemp)) {
		_swprintf_c(targetDir, _countof(targetDir), L"%s", envSnapTemp);
		goto gotADir;
	}
...
gotADir:
	...
	wcscat_s(targetDir, _countof(targetDir), L"\\SnapTemp");
	...
	swprintf_s(logFile, L"%s\\SnapSetup.log", targetDir);

...
cleanup:
  p_ctx->status = static_cast<pal_bsdiff_status_type>(ret);

  if(p_ctx->status == pal_bsdiff_status_type_success) {
  ...
  return p_ctx->status == pal_bsdiff_status_type_success ? TRUE : FALSE;
}

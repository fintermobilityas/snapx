    ...
    const version::Semver200_version & clr_minimum_version)
{
    ...
    pal_file_exists(executable_path.c_str(), &executable_file_exists);
    ...
    if (pal_get_environment_variable(L"CORE_LIBRARIES", &core_libraries_path_out))
    {
...
std::vector<std::wstring> snap::coreclr::get_trusted_platform_assemblies(const wchar_t* trusted_platform_assemblies_path)
{
    ...
    for (const auto& extension : trusted_platform_assemblies_extension_list)
    {
        ...
        if (!pal_list_files(trusted_platform_assemblies_path, nullptr, extension, &files_out, &files_out_len))
        {
            continue;
        }
    ...
    const version::Semver200_version & clr_minimum_version)
{
    ...
    if (core_clr_instance == nullptr && pal_get_environment_variable(core_root_environment_var, &core_root_path_out))
    {
    ...
    if (core_clr_instance == nullptr)
    {
        ...
        if (core_clr_program_files_directory_expanded_length > 0)
        {
            ...
            {
                core_clr_instance = try_load_core_clr(clr_directory.get_dll_path());
                if (core_clr_instance == nullptr)
                {
                    break;
                }
    ...
    const version::Semver200_version& clr_minimum_version)
{
    ...
    for (const auto& core_clr_path : core_clr_paths)
    {
        ...
        if (!pal_get_directory_name(core_clr_path, &clr_directory_core_clrversion, &clr_directory_len))
        {
        ...
        pal_str_convert_from_utf16_to_utf8(clr_directory_core_clrversion, static_cast<int>(clr_directory_len), &multibyte_string_core_clr_version);
        ...
        try
        {
            ...
            core_clr_directories.emplace_back(
                core_clr_directory(core_clr_root_path, core_clr_path, core_clr_version));
        }
...
core_clr_instance* snap::coreclr::try_load_core_clr(const wchar_t* directory_path)
{
    ...
    size_t core_clr_dll_path_len = 0;
    if (!pal_path_combine(directory_path, core_clr_dll, &core_clr_dll_path, &core_clr_dll_path_len))
    {
        return nullptr;
    }

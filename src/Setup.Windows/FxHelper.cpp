...
bool CFxHelper::CanInstallDotNetCore()
{
	return IsWindowsVistaOrGreater();
}

NetCoreAppVersion CFxHelper::GetRequiredDotNetCoreVersion()
{
	...
	//if (resourceFlag.Compare(L"net451") == 0) return NetCoreAppVersion::net451;
	//if (resourceFlag.Compare(L"net452") == 0) return NetCoreAppVersion::net452;
	//if (resourceFlag.Compare(L"net46") == 0) return NetCoreAppVersion::net46;
	//if (resourceFlag.Compare(L"net461") == 0) return NetCoreAppVersion::net461;
	//if (resourceFlag.Compare(L"net462") == 0) return NetCoreAppVersion::net462;
	//if (resourceFlag.Compare(L"net47") == 0) return NetCoreAppVersion::net47;
	//if (resourceFlag.Compare(L"net471") == 0) return NetCoreAppVersion::net471;
	//if (resourceFlag.Compare(L"net472") == 0) return NetCoreAppVersion::net472;

	//Default to standard netcoreapp2
	return NetCoreAppVersion::netcoreapp20;
}

bool CFxHelper::IsDotNetCoreInstalled(NetCoreAppVersion required)
{
	ATL::CRegKey key;
	...
	if (key.QueryDWORDValue(L"Release", dwReleaseInfo) != ERROR_SUCCESS ||
		dwReleaseInfo < GetDotNetCoreVersionReleaseNumber(required)) {
		return false;
	}
...
int CFxHelper::GetDotNetCoreVersionReleaseNumber(NetCoreAppVersion version)
{
	//switch (version) {
	//case NetCoreAppVersion::net451:
	//	return fx451ReleaseVersion;
	//case NetCoreAppVersion::net452:
	//	return fx452ReleaseVersion;
	//case NetCoreAppVersion::net46:
	//	return fx46ReleaseVersion;
	//case NetCoreAppVersion::net461:
	//	return fx461ReleaseVersion;
	//case NetCoreAppVersion::net462:
	//	return fx462ReleaseVersion;
	//case NetCoreAppVersion::net47:
	//	return fx47ReleaseVersion;
	//case NetCoreAppVersion::net471:
	//	return fx471ReleaseVersion;
	//case NetCoreAppVersion::net472:
	//	return fx472ReleaseVersion;
	//case NetCoreAppVersion::net45:
	//default:
...
HRESULT CFxHelper::InstallDotNetCore(NetCoreAppVersion version, bool isQuiet)
{
	if (!isQuiet) {
		CTaskDialog dlg;
	...
	if (!isQuiet) {
		...
		if (pd != nullptr) {
			...
			pd->SetLine(1, L"Downloading the .NET Core installer", FALSE, nullptr);
...
UINT CFxHelper::GetInstallerMainInstructionForVersion(NetCoreAppVersion version)
{
	if (version >= NetCoreAppVersion::net47) {
		return IDS_FXINSTRUCTION47;
	}

	if (version >= NetCoreAppVersion::net46) {
		return IDS_FXINSTRUCTION46;
	}
	return IDS_FXINSTRUCTION;
}

UINT CFxHelper::GetInstallerContentForVersion(NetCoreAppVersion version)
{
	if (version >= NetCoreAppVersion::net47) {
...
UINT CFxHelper::GetInstallerExpandedInfoForVersion(NetCoreAppVersion version)
{
	if (version >= NetCoreAppVersion::net47) {
...
UINT CFxHelper::GetInstallerUrlForVersion(NetCoreAppVersion version)
{
	if (version >= NetCoreAppVersion::net47) {
...
bool CFxHelper::WriteRunOnceEntry()
{
	...
	if (key.SetStringValue(L"SnapInstall", exePath) != ERROR_SUCCESS) {
		return false;
	}

...
public:
    ...
    pal_string(TStorageClass *str_ptr, const bool free) : m_ptr(free ? str_ptr : nullptr),
    ...
    friend std::basic_ostream<TStorageClass> &operator<<(std::basic_ostream<TStorageClass> &out,
                                                         const pal_string &pal_string) {
...
class pal_utf8_string final : public pal_string<char, std::string> {
    pal_utf8_string(char *utf8_string, const bool free) : pal_string<char, std::basic_string<char> >(
        utf8_string, free) {
    }
...
public:
    ...
    explicit pal_utf8_string(const std::wstring &utf16_string) : pal_utf8_string(
        pal_str_narrow(utf16_string.data()), true) {
    }
...
class pal_utf16_string final : public pal_string<wchar_t, std::wstring> {
    pal_utf16_string(wchar_t *utf16_string, const bool free) : pal_string<wchar_t, std::basic_string<wchar_t> >(
        utf16_string, free) {
    }
...
public:
    ...
    explicit pal_utf16_string(const std::string &utf8_string) : pal_utf16_string(
        pal_str_widen(utf8_string.data()), true) {
    }

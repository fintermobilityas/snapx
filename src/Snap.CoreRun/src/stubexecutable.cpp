...
int snap::stubexecutable::run(std::vector<std::string> arguments,
                              const std::map<std::string, std::string> &environment_variables, const int cmd_show) {
    auto exit_code = 1;
    ...
    if (pal_process_daemonize(executable_full_path.c_str(), app_dir_str.c_str(), static_cast<int>(argc), argv, cmd_show,
                              &process_pid)) {
...
std::string snap::stubexecutable::find_current_app_dir() {
    ...
    for (const auto &full_path: paths) {
        ...
        try {
            ...
            LOGE << "Semver parse error! App version: " << current_app_ver_str
                 << ". Full path: " << full_path << ". Exception message: " << e.what();
            continue;
        }
    ...
    if (!pal_path_combine(app_dir.c_str(), app_dir_version_str.c_str(), final_dir.get())) {
        LOGE << "Error! Unable to build final dir. App dir: " << app_dir
             << ". App dir version: " << app_dir_version_str;
        return {};
    }

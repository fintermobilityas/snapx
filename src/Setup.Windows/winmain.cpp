                      ...
                      _In_ int nCmdShow)
{
	...
	if (!CFxHelper::CanInstallDotNetCore()) {
	...
	NetVersion requiredVersion = CFxHelper::GetRequiredDotNetCoreVersion();

	if (!CFxHelper::IsDotNetCoreInstalled(requiredVersion)) {
		hr = CFxHelper::InstallDotNetCore(requiredVersion, isQuiet);
		if (FAILED(hr)) {
			exitCode = hr; // #yolo
			CUpdateRunner::DisplayErrorMessage(CString(L"Failed to install the .NET Core, try installing the latest version manually"), NULL);
			goto out;
		}
